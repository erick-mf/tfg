...
const fetchOrders = async () => {
    try {
        const response = await axios.get(route('kitchen.orders.data'));
...
function toggleAvailability(menuItem) {
    router.put(
        route('kitchen.menu-items.toggle', menuItem.id),
        {},
        {
            preserveState: true,
            preserveScroll: true,
        ...
        <header class="border-primary bg-base-100 sticky top-0 z-10 border-b-2 p-3">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    <h1 class="text-lg font-bold">PANEL DE COCINA</h1>
                </div>
                <div class="flex items-center gap-4">
